# based on train_voter with tiny adjustments 
import torch
import time
...
parser.add_argument('--nodes', type=int, default=25, help='Number of nodes, default=10')
parser.add_argument('--network', type=str, default='ER', help='type of network')
parser.add_argument('--sys', type=str, default='invvoter', help='simulated system to model,spring or cmn')
...
#set gpu id
#torch.cuda.set_device(args.device_id)  # does not work with cuda somehow
...
# load_data
#if args.sys== 'voter':
train_loader, val_loader, test_loader, object_matrix = load_voter(batch_size=HYP['batch_size'],node_num=args.nodes,network=args.network,exp_id=args.exp_id,modelname=MODELNAME)
...
# model save path
dyn_path = './model/dyn_voter_'.replace('voter', MODELNAME) + args.network + '_' + str(args.nodes) + '_id' + str(args.exp_id) + '.pkl'
gen_path = './model/gen_voter_'.replace('voter', MODELNAME) + args.network + '_' + str(args.nodes) + '_id' + str(args.exp_id) + '.pkl'
adj_path = './model/adj_voter_'.replace('voter', MODELNAME) + args.network + '_' + str(args.nodes) + '_id' + str(args.exp_id) + '.pkl'
