...
parser.add_argument('--adj_filepath', type=str, default='SIS_graph_grid5x5_step_1_expnum_0_adj_matrix.pickle',
                    help='filepath of adj matrix')
parser.add_argument('--ts_filepath', type=str, default='SIS_graph_grid5x5_step_1_expnum_0_time_series.pickle',
                    help='filepath of TS data')
parser.add_argument('--predicted_matrix_path', type=str, default='final_predicted_matrix.txt',
                    help='filepath of TS data')
...
adj_filepath = args.adj_filepath
ts_filepath = args.ts_filepath
predicted_matrix_path = args.predicted_matrix_path


print("Data Loading...")
# get data
train_data_loader,valid_data_loader,test_data_loader,edges_train = load_bn_ggn(adj_filepath, ts_filepath, batch_size=Batch_Size, dyn_type = Dyn_Type)
...
print('Adj matrix:')
print(edges_train)
np.savetxt('current_gt_matrix.txt', edges_train, fmt='%i')
print('train_data_loader: ', train_data_loader.dataset[0][0].shape, train_data_loader.dataset[0][1].shape)
print(train_data_loader.dataset[50][0])
print(train_data_loader.dataset[50][1])
print('-----')
...
def train_batch_dyn(optimizer,dyn_learner,adj,data_train,data_target,loss_fn):
    ...
    #print('data_train: ', data_train.shape, ' output: ', output.shape, ' data_target: ',data_target.shape)
...
for epoch in range(Epoch_Num):
    ...
    for i in range(Net_Steps):
        ...
        np.savetxt(predicted_matrix_path, out_matrix.cpu().detach().numpy(), fmt='%i')
    # validate
    # dyn_learner.eval()
    val_losses = []
    print("validating")
...
np.savetxt(predicted_matrix_path+'.final.txt', out_matrix.cpu().detach().numpy(), fmt='%i')
